-- ==============================================================================
-- Migration: 20260527000006_unified_booking_quotes_contracts.sql
-- Unificación total: Reserva pública / en línea → Cliente + Cotización + Contrato/Evento
-- ==============================================================================

-- ── 1. Nuevo Trigger BEFORE INSERT: Reserva → Cliente + Cotización ─────────────
--
-- Se ejecuta BEFORE INSERT en private_reservations para cualquier reserva en línea.
-- Si la reserva no tiene quote_id (reserva directa o cotización en línea),
-- automáticamente:
--   a. Busca o registra al cliente en clients por correo electrónico.
--   b. Crea una cotización en quotes en estado 'borrador' con los montos y el paquete.
--   c. Vincula el quote_id and client_id a la nueva reserva.
-- Esto asegura que "en cuanto coticen", se crea el cliente y la cotización en el CRM.
...
CREATE TRIGGER trg_reservation_on_insert
  BEFORE INSERT ON private_reservations
  FOR EACH ROW
  EXECUTE FUNCTION fn_reservation_on_insert();


-- ── 2. Actualizar función BEFORE UPDATE/INSERT: Confirmada → Contrato ─────────
--
-- Se redefine fn_reservation_confirmed_to_contract para que actúe tanto
-- en INSERT (si se crea confirmada directamente) como en UPDATE (cuando cambia a 'confirmed').
-- Si ya tiene una cotización en quotes, la marca como 'aprobada' y crea el contrato.
...
CREATE TRIGGER trg_reservation_confirmed
  BEFORE INSERT OR UPDATE ON private_reservations
  FOR EACH ROW
