...
export class AdminEventDetail {
  ...
  private buildContractHtml(c: Contract, approvedAmendments: QuoteAmendment[]): string {
    ...
    const pkgPrice = this.getContractPackagePrice(quoteData);
    ...
    const activitiesHtml = formatPrintList(this.getContractActivitiesWithPrices(quoteData));
    const decorationsHtml = formatPrintList(this.getContractDecorationsWithPrices(quoteData));
    const glamHtml = formatPrintList(this.getContractGlamWithPrices(quoteData));
    const extrasHtml = formatPrintList(this.getContractExtrasWithPrices(quoteData));

    let snackPriceHtml = '—';
    if (snack !== '—') {
      const snackPrice = this.getContractSnackPrice(quoteData);
    ...
    return `<!DOCTYPE html><html lang="es"><head>
      ...
      <table class="details-table">
        ...
        <tr>
          ...
          <td colspan="2">De ${c.hora_inicio ? this.formatTime(c.hora_inicio) : '—'} a ${c.hora_fin ? this.formatTime(c.hora_fin) : '—'}</td>
        </tr>
        <tr>
